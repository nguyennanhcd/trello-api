...
const createNew = async (data) => {
  try {
    ...
    const createdColumn = await GET_DB().collection(COLUMN_COLLECTION_NAME).insertOne({ ...validData, boardId: new ObjectId( String(validData.boardId)) })
...
const update = async (columnId, updateData) => {
  try {
    ...
    {
      updateData.cardOrderIds = updateData.cardOrderIds.map(cardId => new ObjectId(String(cardId)))
    }
